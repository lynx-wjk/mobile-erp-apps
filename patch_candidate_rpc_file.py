...
with open('patch_candidate_rpc.sql', 'w', encoding='utf-8') as f:
    f.write(sql_content)

# Upload via SCP to VPS
subprocess.run(['scp', 'patch_candidate_rpc.sql', 'inventory-vps:/tmp/patch_candidate_rpc.sql'], check=True)

# Run psql on VPS
res = subprocess.run(['ssh', 'inventory-vps', 'docker exec -i supabase-db psql -U postgres -d postgres -f /tmp/patch_candidate_rpc.sql'], text=True, capture_output=True)
print("PSQL Output:", res.stdout)
if res.stderr:
    print("PSQL Stderr:", res.stderr)
