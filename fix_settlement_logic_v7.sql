-- Fix settlement logic for TikTok Shop and Shopee in finance_dashboard_snapshot_base_20260623
-- COGS/HPP is only included in Settled HPP if payout data is confirmed settled (nullif(trim(settlement_status),'') is not null and not unsettled).
-- All orders where payout is not yet recorded/settled have their COGS categorized under "est. hpp belum payout".
...
begin
  ...
  -- 2. Finance Settlement Metrics from marketplace_finance_reports (Confirmed Settled Payouts only)
  select
  ...
  where coalesce(fr.period_start::date, fr.settlement_date::date, fr.created_at::date) between v_start and v_end
    ...
    and nullif(trim(fr.settlement_status), '') is not null
    and lower(trim(fr.settlement_status)) not in ('unsettled', 'pending', 'waiting_settlement', 'belum_payout', 'belum_cair')
    and (
      v_marketplace is null
      or case
  ...
  -- 4. Calculate Estimated HPP Belum Payout (Orders where payout data is not yet recorded or settled)
  with settled_target_fr as (
    select fr.marketplace_order_id, fr.order_id
    from public.marketplace_finance_reports fr
    where fr.tenant_id = v_tenant_id
      and coalesce(fr.period_start::date, fr.settlement_date::date, fr.created_at::date) between v_start and v_end
      and fr.order_id is not null and nullif(trim(fr.order_id), '') is not null
  ...
  -- 6. Build Marketplace Cards with Dynamic HPP for Settled Orders
  ...
  hpp_target_fr as (
    ...
    where fr2.tenant_id = v_tenant_id
      ...
      and nullif(trim(fr2.settlement_status), '') is not null
      and lower(trim(fr2.settlement_status)) not in ('unsettled', 'pending', 'waiting_settlement', 'belum_payout', 'belum_cair')
      and (
        v_marketplace is null
        or case
