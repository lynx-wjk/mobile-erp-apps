-- Fix date filtering in finance_dashboard_snapshot_base_20260623 and finance_dashboard_snapshot wrapper
-- Order/period date (period_start) is prioritized so historical imports/reports from previous months do not spill into current month reports.
...
begin
  ...
  select coalesce(sum(order_count), 0)::integer into v_order_count from rows;
