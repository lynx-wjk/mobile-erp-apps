...
async function resolveLocalSkuForFinance(serviceClient, account, row, orderId = '') {
  ...
  if (safeOrderId && itemFilters) {
    const { data } = await serviceClient.from('marketplace_order_items').select('marketplace_sku_map_id, product_id, mapped_product_id, local_product_id, local_sku, mapped_local_sku, tracking_number').eq('tenant_id', tenantId).eq('marketplace_account_id', accountId).or(`order_sn.eq.${safeOrderId},external_order_id.eq.${safeOrderId},order_id.eq.${safeOrderId},remote_order_id.eq.${safeOrderId}`).or(itemFilters).limit(1).maybeSingle();
    orderItem = data;
  }
  ...
  if (!orderItem && safeOrderId && !remoteSkuId && !sellerSku) {
    const { count } = await serviceClient.from('marketplace_order_items').select('marketplace_order_item_id', {
      count: 'exact',
      head: true
    }).eq('tenant_id', tenantId).eq('marketplace_account_id', accountId).or(`order_sn.eq.${safeOrderId},external_order_id.eq.${safeOrderId},order_id.eq.${safeOrderId},remote_order_id.eq.${safeOrderId}`);
    if ((count ?? 0) === 1) {
      const { data } = await serviceClient.from('marketplace_order_items').select('marketplace_sku_map_id, product_id, mapped_product_id, local_product_id, local_sku, mapped_local_sku, tracking_number').eq('tenant_id', tenantId).eq('marketplace_account_id', accountId).or(`order_sn.eq.${safeOrderId},external_order_id.eq.${safeOrderId},order_id.eq.${safeOrderId},remote_order_id.eq.${safeOrderId}`).limit(1).maybeSingle();
      orderItem = data;
    }
  }
  if (!orderItem) return {};
...
async function saveFinanceStatementReport(serviceClient, account, statement, rawPayload) {
  ...
  const statementTime = isoFromFinanceTime(statement.statement_time ?? statement.create_time ?? statement.created_time ?? statement.settlement_time);
  const statementDate = (statementTime ?? new Date().toISOString()).slice(0, 10);
...
async function saveFinanceItemFromRow(args) {
  ...
  const remoteSkuId = getString(detailRow.sku_id ?? detailRow.remote_sku_id ?? detailRow.product_sku_id, '');
  ...
  const gross = moneyByKeys(detailRow, [
    'gross_sales',
    'gross_amount',
    'revenue',
    'revenue_amount',
  ...
  const payout = moneyByKeys(detailRow, [
    'settlement',
    'settlement_amount',
    'total_settlement_amount',
    'payout',
  ...
  const platformFee = Math.abs(moneyByKeys(detailRow, [
    'platform_fee',
    'platform_service_fee',
    'transaction_fee'
  ]));
  const commissionFee = Math.abs(moneyByKeys(detailRow, [
    'commission_fee',
    'referral_fee'
  ]));
  const affiliateFee = Math.abs(moneyByKeys(detailRow, [
    'affiliate_commission',
    'affiliate_fee',
    'affiliate_partner_commission',
    'affiliate_shop_ads_commission'
  ]));
  const shippingFee = Math.abs(moneyByKeys(detailRow, [
    'shipping_fee',
    'shipping_cost',
    'tiktok_shop_shipping_fee'
  ]));
  const refundAmount = Math.abs(moneyByKeys(detailRow, [
    'refund_amount',
    'gross_sales_refund',
    'customer_refund'
  ]));
  const discountAmount = Math.abs(moneyByKeys(detailRow, [
    'discount_amount',
    'seller_discount',
    'seller_cofunded_voucher_discount'
  ]));
  const map = await resolveLocalSkuForFinance(serviceClient, account, detailRow, orderId);
  ...
  const payload = {
    ...
    local_sku: getString(map.local_sku ?? sellerSku ?? remoteSkuId, ''),
    marketplace_sku: remoteSkuId || null,
