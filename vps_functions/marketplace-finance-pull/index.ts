...
async function pullTikTokFinanceForAccount(
  ...
  const todayJakarta = jakartaDateString(0);
  const fallbackStart = jakartaDateString(-Math.min(Math.max(args.daysBack, 1), 90));
  const startDate = args.startDate || fallbackStart;
  const endDate = args.endDate || todayJakarta;

  // TikTok payout dashboard source of truth is marketplace_finance_items.
  // The previous process_finance_sync_jobs path mostly writes marketplace_finance_reports
  // and can be killed by Edge supervisor on bigger batches.
  // Probe confirmed pull_finance_statements_period with small chunks succeeds.
  const maxTransactions = 20;
  const maxOrderDetails = 6;
  ...
  const response = await invokeTikTokService(supabaseUrl, cronSecret, {
    ...
    params: {
      ...
      source: "marketplace_finance_pull_tiktok_statement_items_safe_v1",
    },
  });

  const body = response.body || {};
  const checked = numberValue(body.transactions ?? body.checked);
  const synced = numberValue(body.items ?? body.payout_success ?? body.success);
  const failed = response.status >= 200 && response.status < 300 && body?.ok === true ? 0 : 1;

  const blocked = !(response.status >= 200 && response.status < 300 && body?.ok === true);
  const waitingSettlement = !blocked && checked > 0 && synced <= 0;

  const message = blocked
    ? `TikTok finance statement gagal: service_status=${response.status}.`
    : `TikTok finance statement selesai: statements=${numberValue(body.statements)}, transactions=${checked}, items=${synced}.`;

  return {
    account_id: account.marketplace_account_id,
    store_alias: account.store_alias || account.shop_name || account.shop_id,
    checked,
    synced,
    failed,
    jobs: numberValue(body.statements),
    ok: !blocked,
    blocked,
    waiting_settlement: waitingSettlement,
    error_code: blocked ? "tiktok_finance_statement_not_synced" : undefined,
    message,
