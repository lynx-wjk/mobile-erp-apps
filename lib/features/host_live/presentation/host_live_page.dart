...
class _HostLivePageState extends State<HostLivePage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final dynamic rawData = _canManage
          ? await query.order('tanggal', ascending: false).order('created_at', ascending: false).limit(250)
          : await query.eq('user_id', authUser.id).order('tanggal', ascending: false).order('created_at', ascending: false).limit(250);
  ...
  List<String> _statusOptions() {
    if (_canManage) return const ['scheduled', 'live_started', 'finished', 'verified', 'rejected'];
  ...
  Future<void> _updateStatus(Map<String, dynamic> item, String status) async {
    try {
      await _client.from('live_schedules').update({
        'status': status,
  ...
  Future<void> _deleteSchedule(Map<String, dynamic> schedule) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text("Jadwal live ${AppUi.text(schedule['host_name'] ?? schedule['users']?['nama'])} akan dihapus."),
        actions: [
  ...
  Future<void> _showAddForm() async {
    if (!_canManage) {
      AppUi.showSnack('Hanya HR dan Super Admin yang bisa membuat jadwal live.');
      return;
    }

    String? hostId = _users.isEmpty ? null : _users.first['user_id']?.toString();
        ...
        return StatefulBuilder(
          builder: (context, setSheetState) {
            Future<void> save() async {
              ...
              final host = _users.firstWhere((user) => user['user_id']?.toString() == hostId, orElse: () => const {});
              final hostName = host['nama']?.toString() ?? host['email']?.toString() ?? '-';
              ...
              try {
                await _client.from('live_schedules').insert({
                  ...
                  'host_id': hostId,
                  'host_name': hostName,
                  'title': titleController.text.trim().isEmpty ? 'Live Session' : titleController.text.trim(),
                  ...
                  'platform': platformController.text.trim().isEmpty ? 'TikTok' : platformController.text.trim(),
                  'status': 'scheduled',
            ...
            return Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text('Tambah Jadwal Host Live', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
                    const SizedBox(height: 14),
                    DropdownButtonFormField<String>(
                      value: hostId,
                      decoration: const InputDecoration(labelText: 'Host', border: OutlineInputBorder()),
                      items: _users.map((user) => DropdownMenuItem<String>(value: user['user_id']?.toString(), child: Text('${user['nama'] ?? '-'} • ${user['role_id'] ?? '-'}'))).toList(),
                      onChanged: (value) => setSheetState(() => hostId = value),
                    ),
                    const SizedBox(height: 12),
                    TextField(controller: titleController, decoration: const InputDecoration(labelText: 'Judul live', border: OutlineInputBorder())),
                    const SizedBox(height: 12),
                    TextField(controller: platformController, decoration: const InputDecoration(labelText: 'Platform', border: OutlineInputBorder())),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                        Expanded(child: TextField(controller: shiftController, decoration: const InputDecoration(labelText: 'Shift', border: OutlineInputBorder()))),
                        const SizedBox(width: 10),
                        Expanded(child: TextField(controller: sesiController, decoration: const InputDecoration(labelText: 'Sesi', border: OutlineInputBorder()))),
                      ],
                    ),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                        Expanded(child: TextField(controller: startController, decoration: const InputDecoration(labelText: 'Jam mulai', border: OutlineInputBorder()))),
                        const SizedBox(width: 10),
                        Expanded(child: TextField(controller: endController, decoration: const InputDecoration(labelText: 'Jam selesai', border: OutlineInputBorder()))),
                      ],
                    ),
                    const SizedBox(height: 12),
                    OutlinedButton.icon(
                    ...
                    FilledButton.icon(onPressed: save, icon: const Icon(Icons.save_outlined), label: const Text('Simpan Jadwal')),
                  ],
                ),
              ),
            );
        ...
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(item['title']?.toString() ?? 'Live Session', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16)),
                    const SizedBox(height: 4),
                    Text('Host: ${item['host_name'] ?? '-'}'),
                    Text('${AppUi.date(item['tanggal'])} • ${item['shift'] ?? '-'} • ${item['sesi'] ?? '-'}'),
                    Text('${item['jam_mulai'] ?? '-'} - ${item['jam_selesai'] ?? '-'}'),
                  ],
                ),
              ),
          ...
          Row(
            children: [
              ...
              Expanded(child: Text(proofUrl.trim().isEmpty ? 'Bukti stream off belum diupload' : 'Bukti stream off tersedia')),
              DropdownButton<String>(
                value: _statusOptions().contains(status) ? status : null,
                hint: const Text('Status'),
                underline: const SizedBox.shrink(),
                items: _statusOptions().map((status) => DropdownMenuItem(value: status, child: Text(_statusLabel(status)))).toList(),
                onChanged: (value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: _canManage
          ? FloatingActionButton.extended(onPressed: _showAddForm, icon: const Icon(Icons.add), label: const Text('Jadwal'))
          : null,
      body: _isLoading
          ? const LoadingState()
          : _errorMessage != null
                    ...
                    children: [
                      FuturisticHeader(
                        ...
                        stats: [
                          ...
                          StatPill(label: 'Finished', value: _items.where((item) => item['status'] == 'finished' || item['status'] == 'verified').length.toString()),
                          StatPill(label: 'Proof', value: _items.where((item) => ((item['proof_photo_url'] ?? '').toString()).isNotEmpty).length.toString()),
                        ],
                      ),
                      const SizedBox(height: 16),
                      if (_items.isEmpty)
                        const EmptyState(title: 'Belum ada jadwal live', subtitle: 'Jadwal host live akan tampil di sini.')
                      else
...
class _HostLiveDetailPageState extends State<HostLiveDetailPage> {
  ...
  List<String> _statusOptions() {
    if (widget.canManage) return const ['scheduled', 'live_started', 'finished', 'verified', 'rejected'];
    return const ['live_started', 'finished'];
  }
        ...
        body: ListView(
          ...
          children: [
            FuturisticHeader(
              ...
              subtitle: 'Detail live, status, dan bukti stream off berbasis lokasi.',
              stats: [
            ...
            NiceCard(
              child: Column(
                ...
                children: [
                  ...
                  _row('Jam', '${_item['jam_mulai'] ?? '-'} - ${_item['jam_selesai'] ?? '-'}'),
            ...
            EvidenceCameraField(
              ...
              helperText: 'Wajib berada dalam radius lokasi kerja aktif. GPS, tanggal, dan jam tersimpan otomatis.',
