-- v9b_optimized: Fix HPP overcounting + fast execution
-- Only count HPP for orders with payout > 0, exclude cancelled
-- Use JOIN-based exclusion instead of NOT EXISTS for performance
...
declare
  ...
  -- v9b: Arrays for paid order IDs (payout > 0) for fast hash lookups
  v_paid_order_ids text[];
  v_paid_mp_order_ids text[];
...
begin
  ...
  where (o.order_created_at::date between v_start and v_end or (o.order_created_at is null and o.created_at::date between v_start and v_end))
    and o.tenant_id = v_tenant_id
    and (p_account_id is null or o.marketplace_account_id = p_account_id)
    and not (lower(concat_ws(' ', o.order_status, o.status, coalesce(o.raw_order->>'status', ''))) ~ '(cancel|canceled|cancelled|batal|dibatalkan|returned|return|refund|rts|gagal|failed|closed)')
    and (
  ...
  -- 2. Finance Settlement Metrics (total payout includes all records: positive, zero, negative)
  select
  ...
  where (fr.settlement_date between v_start and v_end or (fr.settlement_date is null and fr.created_at::date between v_start and v_end))
  ...
  -- 2b. Collect PAID order IDs (payout > 0) into arrays for fast hash lookups
  select
    array_remove(array_agg(distinct fr.order_id), null),
    array_remove(array_agg(distinct fr.marketplace_order_id::text), null)
  into v_paid_order_ids, v_paid_mp_order_ids
  from public.marketplace_finance_reports fr
  where (fr.settlement_date between v_start and v_end or (fr.settlement_date is null and fr.created_at::date between v_start and v_end))
    and fr.tenant_id = v_tenant_id
    and (p_account_id is null or fr.marketplace_account_id = p_account_id)
  ...
  -- 3. Dynamic HPP for Settled Orders (ONLY payout > 0 + NOT cancelled)
  with hpp_sku as (
  ...
  settled_orders as (
    ...
    where o.tenant_id = v_tenant_id
      and (p_account_id is null or o.marketplace_account_id = p_account_id)
      and (
        (v_paid_order_ids is not null and o.order_id = ANY(v_paid_order_ids))
        or (v_paid_mp_order_ids is not null and o.marketplace_order_id::text = ANY(v_paid_mp_order_ids))
      )
      -- v9 FIX: Exclude cancelled/returned orders
      and not (lower(concat_ws(' ', o.order_status, o.status, coalesce(o.raw_order->>'status', ''))) ~ '(cancel|canceled|cancelled|batal|dibatalkan|returned|return|refund|rts|gagal|failed|closed)')
  )
  ...
  -- 4. Est. HPP Belum Payout: Orders created in date range that have NO payout > 0
  --    Includes: no finance record at all, OR finance record with payout = 0
  ...
  unpaid_orders as (
    ...
    where o.tenant_id = v_tenant_id
      and (p_account_id is null or o.marketplace_account_id = p_account_id)
      and (o.order_created_at::date between v_start and v_end or (o.order_created_at is null and o.created_at::date between v_start and v_end))
      -- Exclude cancelled/returned orders entirely
      and not (lower(concat_ws(' ', o.order_status, o.status, coalesce(o.raw_order->>'status', ''))) ~ '(cancel|canceled|cancelled|batal|dibatalkan|returned|return|refund|rts|gagal|failed|closed)')
      -- Exclude orders that already have payout > 0 (using fast array hash lookup)
      and not (
        (v_paid_order_ids is not null and o.order_id = ANY(v_paid_order_ids))
        or (v_paid_mp_order_ids is not null and o.marketplace_order_id::text = ANY(v_paid_mp_order_ids))
      )
  ...
  -- 7. Marketplace Breakdown (v9: same payout > 0 + exclude cancelled filters)
  with mp_orders as (
    select
  ...
  select jsonb_agg(
    jsonb_build_object(
      ...
      'gross_sales', mo.omzet,
      'payout_total', coalesce(mf.payout, 0),
      'payout', coalesce(mf.payout, 0),
      'net_settlement', coalesce(mf.payout, 0),
      'received_amount', coalesce(mf.payout, 0),
      'hpp_total', coalesce(mh.hpp, 0),
      'hpp', coalesce(mh.hpp, 0),
      'commission_fee', coalesce(mf.commission_fee, 0),
      'platform_fee', coalesce(mf.platform_fee, 0),
      'affiliate_fee', coalesce(mf.affiliate_fee, 0),
      'shipping_fee', coalesce(mf.shipping_fee, 0),
      'discount_amount', coalesce(mf.discount_amount, 0),
      'voucher_amount', coalesce(mf.discount_amount, 0),
      'refund_amount', coalesce(mf.refund_amount, 0),
      'adjustment_amount', coalesce(mf.adjustment_amount, 0),
      'fee_amount', coalesce(mf.fee_amount, 0),
      'total_fees', coalesce(mf.fee_amount, 0),
...
begin
  if v_tenant_id is null then
    v_tenant_id := 'ae730499-550b-4907-bb18-bbc2629c64f4'::uuid;
  end if;
  return public.finance_dashboard_snapshot_base_20260623(v_tenant_id, p_start, p_end, p_marketplace, p_account_id);
end;
$function$;
