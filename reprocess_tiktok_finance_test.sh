...
if [ -z "$ACCOUNT_ID" ]; then
  echo "ERROR: TikTok active account tidak ketemu"
  exit 1
fi
...
if [ ! -f "$ENV_FILE" ]; then
  echo "ERROR: env file tidak ketemu"
  exit 1
fi
...
if [ -z "$CRON_SECRET" ]; then
  echo "ERROR: MARKETPLACE_CRON_SECRET tidak ketemu di $ENV_FILE"
  exit 1
fi
...
echo "INVOKE marketplace-tiktok-service with cron secret..."
