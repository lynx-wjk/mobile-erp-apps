...
(
fr.finance_report_id is not null
and (
case
when lower(coalesce(u.marketplace, '')) in ('shopee', 'shopee_normal') then
not (upper(coalesce(fr.settlement_status, '')) ~ '(SHIPPED|DIKIRIM|RECEIVE|BELUM|PENDING|UNPAID|PERLU|READY|BATAL|CANCEL|RETURN|DITERIMA|TERIMA)')
else
(coalesce(fr.payout_amount, fr.received_amount, fr.net_settlement, 0) <> 0
or upper(coalesce(fr.settlement_status, '')) ~ '(SETTLED|PAID|RELEASE|PAYOUT_MINUS|NEGATIVE_PAYOUT)')
end
)
) as has_payout,
...
union all
select
null::uuid as finance_report_id,
...
'SETTLED'::text as settlement_status,
3 as priority
from public.marketplace_finance_items fi
where fi.tenant_id = u.tenant_id
and fi.marketplace_account_id = u.marketplace_account_id
and coalesce(nullif(fi.order_id, ''), nullif(fi.order_sn, ''), nullif(fi.external_order_id, '')) = u.order_key
and lower(u.marketplace) in ('tiktok', 'tiktok_shop')
having count(*) > 0
) fr
order by priority
limit 1
...
and (
case
when lower(coalesce(m.marketplace, '')) in ('shopee', 'shopee_normal') then
not (upper(coalesce(fr.settlement_status, '')) ~ '(SHIPPED|DIKIRIM|RECEIVE|BELUM|PENDING|UNPAID|PERLU|READY|BATAL|CANCEL|RETURN|DITERIMA|TERIMA)')
else
(coalesce(fr.payout_amount, fr.received_amount, fr.net_settlement, 0) <> 0
or upper(coalesce(fr.settlement_status, '')) ~ '(SETTLED|PAID|RELEASE|PAYOUT_MINUS|NEGATIVE_PAYOUT)')
...
and fi.marketplace_account_id = m.marketplace_account_id
and coalesce(nullif(fi.order_id, ''), nullif(fi.order_sn, ''), nullif(fi.external_order_id, '')) = m.order_key
and lower(m.marketplace) in ('tiktok', 'tiktok_shop')
having count(*) > 0
) fr
order by priority
limit 1
