do $$
begin
  if to_regprocedure('public.finance_sku_order_details_group_20260625(date,date,text,uuid,text,text,text,text,integer,integer)') is null then
    alter function public.finance_sku_order_details(date,date,text,uuid,text,text,text,text,integer,integer)
      rename to finance_sku_order_details_group_20260625;
  end if;
end $$;
...
begin
  ...
  -- 1) Coba core lama dulu. Kalau ada rows, jangan ubah perilaku detail existing.
  v_core := public.finance_sku_order_details_core_20260625(
    p_start,p_end,p_marketplace,p_account_id,p_marketplace_sku,p_local_sku,p_search,p_payout_filter,p_page,p_page_size
  );

  if coalesce(jsonb_array_length(coalesce(v_core->'rows','[]'::jsonb)),0) > 0 then
    return jsonb_set(
      v_core,
      '{source}',
      to_jsonb((coalesce(v_core->>'source','finance_sku_order_details_core_20260625') || '+core_hit')::text),
      true
    );
  end if;
  ...
  if v_role <> 'service_role' and v_tenant_id is null then
    return jsonb_build_object(
      ...
      'total_count',0,
      'total_pages',1,
      'source','finance_sku_order_details_detail_fallback_no_tenant_20260625'
    );
  end if;
  ...
  line_base as (
    select
      ...
      coalesce(nullif(oi.marketplace_sku_id,''), nullif(oi.remote_sku_id,''), nullif(oi.marketplace_sku,''), nullif(oi.marketplace_seller_sku,''), nullif(oi.seller_sku,'')) as marketplace_sku,
      ...
      concat_ws(' ',
        ...
        oi.marketplace_product_name, oi.product_name,
        oi.marketplace_variant_name, oi.variant_name, oi.variation_name
      ) as search_blob
    from order_base ob
    ...
    where
      ...
      or (
        v_lsku is not null
        and v_lsku in (
          lower(coalesce(oi.local_sku,'')),
          lower(coalesce(oi.mapped_local_sku,''))
        )
      )
      or (
  ...
  select jsonb_build_object(
    'rows',
      coalesce(jsonb_agg(jsonb_build_object(
        'source','finance_sku_order_details_detail_fallback_20260625',
        'order', order_key,
        'order_id', order_key,
        'order_sn', order_sn,
        ...
        'status', case when coalesce(order_payout,0) > 0 then 'paid' else 'unpaid' end,
        'payout_status', case when coalesce(order_payout,0) > 0 then 'SETTLED' else 'PENDING_PAYOUT' end,
        'hpp', 0,
        'hpp_total', 0,
        'total_hpp', 0,
        'profit', line_payout,
        'margin_percent', case when line_gross > 0 then round((line_payout / line_gross) * 100, 2) else 0 end
    ...
    'core_rows', coalesce(jsonb_array_length(coalesce(v_core->'rows','[]'::jsonb)),0),
  ...
  return coalesce(v_result, jsonb_build_object(
    ...
    'total_count',0,
    'total_pages',1,
    'source','finance_sku_order_details_detail_fallback_20260625_empty',
    'core_source',coalesce(v_core->>'source',''),
    'core_rows',coalesce(jsonb_array_length(coalesce(v_core->'rows','[]'::jsonb)),0),
    'debug_params', jsonb_build_object(
      'marketplace', p_marketplace,
