-- Migration: 20260724170000_fix_overlay_items_direct_join.sql
-- Direct join marketplace_order_items on marketplace_order_id in overlay RPC
...
begin
  ...
  order_items_enrich as (
    ...
    left join hpp_sku hs on hs.tenant_id = opm.tenant_id and hs.marketplace_account_id = opm.marketplace_account_id and hs.marketplace_sku_id = lower(nullif(oi.marketplace_sku_id, ''))
    left join hpp_seller hsel on hsel.tenant_id = opm.tenant_id and hsel.marketplace_account_id = opm.marketplace_account_id and hsel.marketplace_seller_sku = lower(nullif(oi.marketplace_seller_sku, ''))
    left join hpp_local hl on hl.tenant_id = opm.tenant_id and hl.marketplace_account_id = opm.marketplace_account_id and hl.local_sku = lower(nullif(coalesce(oi.mapped_local_sku, oi.local_sku), ''))
  ),
  overall_stats as (
    select
  ...
  mp_enriched as (
    select
      ...
      coalesce(p.order_count, (r.elem->>'order_count')::integer, (r.elem->>'orders_count')::integer, 0)::integer as order_count_val
    from mp_rows r
