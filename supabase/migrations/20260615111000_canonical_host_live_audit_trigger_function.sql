-- 9I-G canonical host live audit trigger function.
-- Repoints live_proofs/live_schedules audit triggers from audit_host_live_change_v1() to audit_host_live_change().
-- Old *_v1 function is intentionally kept; no DROP FUNCTION here.

create or replace function public.audit_host_live_change()
returns trigger
language plpgsql
as $function$
declare
...
begin
  for r in
    ...
    from (
      values
        ('live_proofs', 'trg_audit_host_live_proofs'),
        ('live_schedules', 'trg_audit_host_live_schedules')
  ...
  loop
    ...
    v_def := replace(
      v_def,
      'EXECUTE FUNCTION audit_host_live_change_v1()',
      'EXECUTE FUNCTION public.audit_host_live_change()'
    );

    v_def := replace(
      v_def,
      'EXECUTE FUNCTION public.audit_host_live_change_v1()',
      'EXECUTE FUNCTION public.audit_host_live_change()'
    );
