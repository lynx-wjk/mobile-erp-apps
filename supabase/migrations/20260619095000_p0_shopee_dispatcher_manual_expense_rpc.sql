-- P0 bounded patch: harden canonical manual expense edit/delete RPCs.
-- No DROP, no new versioned wrappers, no business-data mutation.
...
AS $function$
begin
  perform public.finance_update_manual_operational_expense(
    p_expense_id,
    p_category,
    p_amount,
    p_expense_date,
...
CREATE OR REPLACE FUNCTION public.finance_delete_manual_operational_expense(p_expense_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
GRANT EXECUTE ON FUNCTION public.finance_update_manual_operational_expense(uuid, text, numeric, date, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.finance_update_manual_operational_expense(uuid, numeric, text, date, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.finance_delete_manual_operational_expense(uuid) TO authenticated, service_role;
