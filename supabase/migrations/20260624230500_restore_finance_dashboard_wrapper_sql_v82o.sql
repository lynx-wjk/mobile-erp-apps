...
declare
  def text;
begin
  if to_regprocedure('public.finance_dashboard_snapshot_wrapper_broken_20260624(date,date,text,uuid)') is null then
    ...
    def := replace(
      ...
      'FUNCTION public.finance_dashboard_snapshot_wrapper_broken_20260624('
    );
...
as $$
  select public.finance_customer_dashboard_snapshot_v24_6_82o(
    p_start,
    p_end,
    p_marketplace,
    p_account_id
