-- Migration: Real Gross Before Discount, Marketplace Discount & Negative Payout Aggregation
-- Migration ID: 20260815224000_real_gross_discount_and_negative_payout.sql

CREATE OR REPLACE FUNCTION public.sync_missing_completed_order_payouts()
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  loop
    ...
    -- Estimate platform fee at ~15% for Shopee / ~18% for TikTok Shop if detailed report is pending API sync
    v_platform_fee := case
    ...
    insert into public.marketplace_finance_reports (
      ...
      v_platform_fee,
      'order_settlement',
      'Auto-synced payout for completed order',
      now(),
      now()
    )
  ...
  return jsonb_build_object(
    ...
    'message', format('Berhasil meng-upsert %s payout order selesai.', v_inserted_count)
  );
end;
$function$;
...
declare
  ...
  v_negative_payout_total numeric := 0;
...
begin
  ...
  with item_agg as (
    select 
      marketplace_order_id,
      sum(coalesce(nullif(gross_amount, 0), quantity * unit_gross_amount, 0))::numeric as total_item_gross
    from public.marketplace_order_items
  ...
  orders_scope as (
    select
      ...
      coalesce(
        nullif(case when o.marketplace ~ 'tiktok' then (o.raw_order->'payment'->>'original_total_product_price')::numeric else null end, 0),
        nullif(ia.total_item_gross, 0),
        nullif(o.gross_amount, 0),
        o.paid_amount,
        o.total_amount,
        0
      )::numeric as gross_amt,
      coalesce(
        nullif(case when o.marketplace ~ 'tiktok' then (o.raw_order->'payment'->>'seller_discount')::numeric else null end, 0),
        greatest(coalesce(nullif(ia.total_item_gross, 0), o.gross_amount, 0) - coalesce(o.paid_amount, o.total_amount, 0), 0),
        0
      )::numeric as discount_amt,
      coalesce(o.paid_amount, o.total_amount, 0)::numeric as paid_amt
    from public.marketplace_orders o
    left join item_agg ia on ia.marketplace_order_id = o.marketplace_order_id
  ...
  mkt_summary as (
    select
      ...
      sum(os.discount_amt) as discount_amount,
      ...
      sum(case when fr.payout < 0 then fr.payout else 0 end) as negative_payout_total,
      count(distinct case when fr.payout < 0 then os.marketplace_order_id end) as negative_payout_count,
  ...
  select
    ...
    coalesce(sum(negative_payout_total), 0),
    coalesce(sum(negative_payout_count), 0)::integer,
    ...
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'gross_before_discount', m.gross_sales,
      'omzet_paid', m.omzet_paid,
      'omzet_normal_paid', m.omzet_paid,
      'seller_discount', m.discount_amount,
      'discount_amount', m.discount_amount,
      'voucher_amount', m.discount_amount,
      ...
      'negative_payout_total', round(abs(m.negative_payout_total), 2),
      'payout_minus_total', round(abs(m.negative_payout_total), 2),
      'payout_minus_total_abs', round(abs(m.negative_payout_total), 2),
      'negative_payout_count', m.negative_payout_count,
      'payout_minus_count', m.negative_payout_count,
  ...
  into
    ...
    v_negative_payout_total, v_negative_payout_count,
  ...
  return jsonb_build_object(
    ...
    'gross_before_discount', v_gross_sales,
    'omzet_paid', v_omzet_paid,
    'omzet_normal_paid', v_omzet_paid,
    'seller_discount', v_discount_amount,
    ...
    'negative_payout_total', round(abs(v_negative_payout_total), 2),
    'payout_minus_total', round(abs(v_negative_payout_total), 2),
    'payout_minus_total_abs', round(abs(v_negative_payout_total), 2),
