...
begin
  drop table if exists pg_temp.finance_dash_order_base;
  drop table if exists pg_temp.finance_dash_finance_by_order;

  create temporary table finance_dash_order_base on commit drop as
  select
  ...
  create index on finance_dash_order_base (marketplace_account_id, order_key, marketplace_key);
  create index on finance_dash_order_base (d, marketplace_key);

  create temporary table finance_dash_finance_by_order on commit drop as
  select
    ob.d,
    ob.marketplace_key,
  ...
  from pg_temp.finance_dash_order_base ob
  left join public.marketplace_finance_reports fr
   ...
   and (
     case
       when public._finance_marketplace_norm_20260624(fr.marketplace) = 'tiktok' then 'tiktok_shop'
  ...
  create index on finance_dash_finance_by_order (d, marketplace_key);
  ...
  orders_daily as (
    ...
    from pg_temp.finance_dash_order_base
    group by d
  ),
  finance_daily as (
    select
    ...
    from pg_temp.finance_dash_finance_by_order
    group by d
  ),
  daily as (
    select
  ...
  breakdown as (
    select
      ...
      case when coalesce(bf.payout_total, 0) > 0 then 100::numeric else 0::numeric end as margin_percent,
  ...
  return jsonb_build_object(
    'ok', true,
    'source', 'finance_dashboard_snapshot_breakdown_temp_table_20260624',
    'start', v_start,
    'end', v_end,
