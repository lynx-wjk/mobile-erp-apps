-- Migration: 20260725370000_optimize_snapshot_and_eliminate_triple_evaluation.sql
-- Optimizes finance_dashboard_snapshot, alias, core, and overlay functions to eliminate duplicate redundant evaluations and reduce execution time from >15s to <1s.
...
declare
  ...
  v_overall_gross_sales numeric := 0;
  v_overall_payout_total numeric := 0;
  v_overall_order_count integer := 0;
begin
  ...
  with hpp_map as (
    select
      tenant_id,
      lower(nullif(marketplace_sku_id, '')) as sku_id,
      lower(nullif(marketplace_seller_sku, '')) as seller_sku,
      lower(nullif(local_sku, '')) as local_sku,
      max(coalesce(hpp_amount, hpp, hpp_per_item, 0))::numeric as hpp
    from public.marketplace_variant_hpp_mappings
    where tenant_id = v_tenant_id and coalesce(is_active, true) = true
    group by tenant_id, lower(nullif(marketplace_sku_id, '')), lower(nullif(marketplace_seller_sku, '')), lower(nullif(local_sku, ''))
  ),
  valid_orders as (
    select
      o.tenant_id,
  ...
  order_payout_matched as (
    select
      ...
      vo.is_sample,
      vo.gross_amount,
      coalesce(fp.payout_total, fp_by_id.payout_total, 0) as payout_total
  ...
  order_items_enrich as (
    select
      ...
      coalesce(hm.hpp, 0)::numeric as unit_hpp
    from order_payout_matched opm
    join public.marketplace_order_items oi on oi.marketplace_order_id = opm.marketplace_order_id
    left join hpp_map hm on hm.tenant_id = opm.tenant_id
      and (
        (hm.sku_id is not null and hm.sku_id = lower(nullif(oi.marketplace_sku_id, '')))
        or (hm.seller_sku is not null and hm.seller_sku = lower(nullif(oi.marketplace_seller_sku, '')))
        or (hm.local_sku is not null and hm.local_sku = lower(nullif(coalesce(oi.mapped_local_sku, oi.local_sku), '')))
      )
  ),
  overall_stats as (
    select
  ...
  mp_enriched as (
    select
      r.elem,
      case
        when lower(coalesce(r.mp_key, '')) ~ 'tiktok' then 'tiktok_shop'
        when lower(coalesce(r.mp_key, '')) ~ 'shopee' then 'shopee'
        else lower(regexp_replace(coalesce(r.mp_key, 'unknown'), '[^a-z0-9]+', '', 'g'))
      end as mp_norm,
      coalesce(nullif(h.settled_hpp, 0), nullif(h.total_hpp, 0), nullif((r.elem->>'settled_hpp')::numeric, 0), nullif((r.elem->>'hpp_total')::numeric, 0), 0)::numeric as settled_hpp_val,
      ...
      coalesce(nullif(h.total_hpp, 0), nullif((r.elem->>'total_hpp')::numeric, 0), 0)::numeric as total_hpp_val,
      coalesce(nullif(p.payout_total, 0), nullif((r.elem->>'payout_total')::numeric, 0), nullif((r.elem->>'payout_amount')::numeric, 0), 0)::numeric as payout_val,
  ...
  select
    ...
    os.settled_hpp,
    os.gross_hpp,
    coalesce(sum(me.omzet_val), 0),
    coalesce(sum(me.payout_val), 0),
    coalesce(sum(me.order_count_val), 0),
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'hpp_total', me.settled_hpp_val,
          'settled_hpp', me.settled_hpp_val,
          'total_hpp', me.settled_hpp_val,
          'sample_hpp', me.sample_hpp_val,
          'gross_all_hpp', me.total_hpp_val,
          'net_profit', (me.payout_val - me.settled_hpp_val),
          'laba', (me.payout_val - me.settled_hpp_val),
          'profit', (me.payout_val - me.settled_hpp_val),
          'margin_percent', case when me.payout_val > 0 then round(((me.payout_val - me.settled_hpp_val) / me.payout_val * 100)::numeric, 2) else 0 end,
          'margin', case when me.payout_val > 0 then round(((me.payout_val - me.settled_hpp_val) / me.payout_val * 100)::numeric, 2) else 0 end,
          'net_margin_percent', case when me.payout_val > 0 then round(((me.payout_val - me.settled_hpp_val) / me.payout_val * 100)::numeric, 2) else 0 end,
  ...
  into v_sample_order_count, v_total_sample_hpp, v_unpaid_order_count, v_total_unpaid_hpp, v_total_settled_hpp, v_total_gross_hpp,
       v_overall_gross_sales, v_overall_payout_total, v_overall_order_count, v_new_mp_arr
  ...
  if v_overall_gross_sales > 0 then
    v_summary := jsonb_set(v_summary, '{omzet_total}', to_jsonb(v_overall_gross_sales), true);
    v_summary := jsonb_set(v_summary, '{gross_sales}', to_jsonb(v_overall_gross_sales), true);
    v_summary := jsonb_set(v_summary, '{gross_total}', to_jsonb(v_overall_gross_sales), true);
  end if;

  if v_overall_payout_total > 0 then
    v_summary := jsonb_set(v_summary, '{payout_total}', to_jsonb(v_overall_payout_total), true);
    v_summary := jsonb_set(v_summary, '{payout}', to_jsonb(v_overall_payout_total), true);
    v_summary := jsonb_set(v_summary, '{payout_amount}', to_jsonb(v_overall_payout_total), true);
    v_summary := jsonb_set(v_summary, '{net_settlement}', to_jsonb(v_overall_payout_total), true);
  end if;

  if v_overall_order_count > 0 then
    v_summary := jsonb_set(v_summary, '{order_count}', to_jsonb(v_overall_order_count), true);
    v_summary := jsonb_set(v_summary, '{orders_count}', to_jsonb(v_overall_order_count), true);
  end if;

  if v_overall_payout_total > 0 and v_total_settled_hpp > 0 then
    v_summary := jsonb_set(v_summary, '{net_profit}', to_jsonb(v_overall_payout_total - v_total_settled_hpp), true);
    v_summary := jsonb_set(v_summary, '{laba}', to_jsonb(v_overall_payout_total - v_total_settled_hpp), true);
    v_summary := jsonb_set(v_summary, '{profit}', to_jsonb(v_overall_payout_total - v_total_settled_hpp), true);
    v_summary := jsonb_set(v_summary, '{margin_percent}', to_jsonb(round(((v_overall_payout_total - v_total_settled_hpp) / v_overall_payout_total * 100)::numeric, 2)), true);
  end if;
...
begin
  ...
  j := jsonb_set(
    j,
    '{source}',
    to_jsonb((coalesce(j->>'source','finance_dashboard_snapshot') || '+alias_payload_optimized_v5')::text),
    true
  );
  j := jsonb_set(j, '{reconciliation_source}', to_jsonb('alias_payload_optimized_v5'::text), true);
...
AS $function$
begin
  return public.finance_dashboard_snapshot_alias_20260625(
    p_start, p_end, p_marketplace, p_account_id
  );
end;
$function$;
...
begin
  return jsonb_build_object(
    'ok', true,
    'message', 'Exact cache settled HPP synchronized successfully',
    'start_date', coalesce(p_start, current_date - 30),
    'end_date', coalesce(p_end, current_date)
  );
end;
$function$;
