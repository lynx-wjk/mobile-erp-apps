-- Fast finance dashboard snapshot.
-- Avoids old heavy base function that times out after TikTok payout report sync.
...
begin
  ...
  order_rows as (
    select
      ...
      )::timestamptz at time zone 'Asia/Jakarta')::date as d,
      o.marketplace_account_id,
      public._finance_marketplace_norm_20260624(to_jsonb(o)->>'marketplace') as marketplace_norm,
      coalesce(nullif(to_jsonb(o)->>'gross_amount','')::numeric, 0) as gross_amount
    from public.marketplace_orders o
    where (coalesce(
        nullif(to_jsonb(o)->>'order_created_at',''),
        nullif(to_jsonb(o)->>'created_at','')
      )::timestamptz at time zone 'Asia/Jakarta')::date between v_start and v_end
  ...
  finance_rows as (
    select
      ...
      public._finance_marketplace_norm_20260624(fr.marketplace) as marketplace_norm,
    ...
    where coalesce(fr.period_start, (fr.created_at at time zone 'Asia/Jakarta')::date) between v_start and v_end
      ...
      and (
        coalesce(trim(p_marketplace), '') = ''
        or public._finance_marketplace_norm_20260624(fr.marketplace)
           = public._finance_marketplace_norm_20260624(p_marketplace)
      )
  ),
  finance_daily as (
  ...
  return jsonb_build_object(
    'ok', true,
    'source', 'finance_dashboard_snapshot_fast_no_base_timeout_20260624',
    'start', v_start,
    'end', v_end,
    'generated_at', now(),
