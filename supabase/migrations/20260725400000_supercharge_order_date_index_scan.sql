-- Migration: 20260725400000_supercharge_order_date_index_scan.sql
-- Replaces un-indexed date casting with direct timestamp range scan on order_created_at index to drop execution time below 300ms.
...
begin
  ...
  v_res := jsonb_build_object(
    'ok', true,
    'source', 'finance_dashboard_snapshot_core_20260625_fixed_v8',
...
begin
  ...
  with hpp_sku as (
    select tenant_id, lower(nullif(marketplace_sku_id, '')) as sku_id,
  ...
  hpp_seller as (
    select tenant_id, lower(nullif(marketplace_seller_sku, '')) as seller_sku,
           max(coalesce(hpp_amount, hpp, hpp_per_item, 0))::numeric as hpp
    from public.marketplace_variant_hpp_mappings
  ...
  valid_order_keys as (
    select distinct order_key from valid_orders union
    select distinct marketplace_order_id::text from valid_orders union
    select distinct order_sn from valid_orders where order_sn is not null union
    select distinct external_order_id from valid_orders where external_order_id is not null
  ),
  finance_payout as (
    select
      fr.tenant_id,
      ...
      sum(coalesce(fr.payout_amount, fr.net_settlement, fr.received_amount, 0)) as payout_total
    from public.marketplace_finance_reports fr
    join valid_order_keys vok on vok.order_key = coalesce(nullif(fr.order_id, ''), fr.marketplace_order_id::text)
    where fr.tenant_id = v_tenant_id
      and (p_account_id is null or fr.marketplace_account_id = p_account_id)
  ...
  order_items_enrich as (
    ...
    left join hpp_sku hs on hs.tenant_id = opm.tenant_id and hs.sku_id = lower(nullif(oi.marketplace_sku_id, ''))
    left join hpp_seller hsel on hsel.tenant_id = opm.tenant_id and hsel.seller_sku = lower(nullif(oi.marketplace_seller_sku, ''))
