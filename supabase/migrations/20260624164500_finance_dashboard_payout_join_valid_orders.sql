-- Dashboard payout must follow valid marketplace_orders in the selected date/account/marketplace filter.
-- This prevents settlement/report rows outside the order filter from inflating payout.
...
begin
  ...
  order_base as (
    select
      ...
      public._finance_marketplace_norm_20260624(o.marketplace) as marketplace_norm,
      coalesce(
  ...
  finance_by_order as (
    ...
    group by ob.d, ob.marketplace_account_id, ob.order_key
  ),
  finance_daily as (
    select
      d,
  ...
  return jsonb_build_object(
    'ok', true,
    'source', 'finance_dashboard_snapshot_payout_join_valid_orders_20260624',
    'start', v_start,
    'end', v_end,
