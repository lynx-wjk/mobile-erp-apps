  ...
  if v_is_unmapped then
    ...
    with_payout as (
      select
        ...
        (fr.finance_report_id is not null and coalesce(fr.payout_amount, fr.received_amount, fr.net_settlement, 0) <> 0) as has_payout
      ...
      left join lateral (
        ...
        from (
          ...
          where fr.tenant_id = u.tenant_id
            ...
            and coalesce(fr.settlement_date, fr.period_start) >= v_start
            and coalesce(fr.settlement_date, fr.period_start) <= v_end
          union all
