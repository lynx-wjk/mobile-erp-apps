...
begin
  ...
  -- TikTok payout total source: statement-level reports only.
  -- Do NOT use all TikTok finance_reports because historical_import/order_settlement rows are inflated.
  select
    coalesce(fr.period_start, (fr.created_at at time zone 'Asia/Jakarta')::date) as d,
    'tiktok_shop'::text as marketplace_key,
    coalesce(fr.received_amount, fr.net_settlement, fr.payout_amount, 0)::numeric as payout_amount
  ...
  -- Non-TikTok stays on finance_reports.
  select
    coalesce(fr.period_start, (fr.created_at at time zone 'Asia/Jakarta')::date) as d,
    case
  ...
  select
    ...
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'fast_snapshot', true,
      'tiktok_payout_from_statement_reports', true
    ) order by d), '[]'::jsonb)
  ...
  return jsonb_build_object(
    'ok', true,
    'source', 'finance_dashboard_tiktok_statement_report_payout_20260624',
    'start', v_start,
    'end', v_end,
