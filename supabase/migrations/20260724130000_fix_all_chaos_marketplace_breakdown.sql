-- Migration: 20260724130000_fix_all_chaos_marketplace_breakdown.sql
-- Resolves all chaos in per-marketplace calculations, order counts, payout, HPP, Laba, and Margin
...
begin
  ...
  order_items_enrich as (
    ...
    join public.marketplace_orders vo on vo.tenant_id = v_tenant_id and coalesce(nullif(vo.external_order_id, ''), nullif(vo.order_sn, ''), vo.marketplace_order_id::text) = opm.order_key
