-- Migration: 20260725220000_fix_overlay_perfect_marketplace_metrics.sql
-- Computes exact distinct HPP, Laba Net, Margin %, and Reconciliation Breakdown per marketplace
...
begin
  ...
  mp_hpp_totals as (
    select
      marketplace_clean as mp_norm,
      coalesce(nullif(sum(qty * unit_hpp) filter (where is_paid and not is_sample), 0), coalesce(sum(qty * unit_hpp), 0))::numeric as settled_hpp,
      coalesce(sum(qty * unit_hpp) filter (where is_sample), 0)::numeric as sample_hpp,
      coalesce(sum(qty * unit_hpp) filter (where not is_paid and not is_sample), 0)::numeric as unpaid_hpp,
