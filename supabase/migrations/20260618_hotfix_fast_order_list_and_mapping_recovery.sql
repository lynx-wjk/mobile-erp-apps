-- Hotfix: robust order list RPC + HPP recovery helpers.
-- Safe for self-host. Does not delete data.
...
create index if not exists idx_mvs_account_variant
  on public.marketplace_variant_snapshots(marketplace_account_id, marketplace_product_id, marketplace_sku_id);
...
begin
  ...
  on conflict (tenant_id, marketplace_account_id, marketplace_product_id, marketplace_sku_id)
  do update set
    local_product_id = excluded.local_product_id,
    local_sku = excluded.local_sku,
    local_product_name = excluded.local_product_name,
    marketplace_seller_sku = coalesce(excluded.marketplace_seller_sku, public.marketplace_variant_hpp_mappings.marketplace_seller_sku),
    marketplace_product_name = coalesce(excluded.marketplace_product_name, public.marketplace_variant_hpp_mappings.marketplace_product_name),
    marketplace_variant_name = coalesce(excluded.marketplace_variant_name, public.marketplace_variant_hpp_mappings.marketplace_variant_name),
    hpp = case when p_overwrite or coalesce(public.marketplace_variant_hpp_mappings.hpp, 0) = 0 then excluded.hpp else public.marketplace_variant_hpp_mappings.hpp end,
    hpp_amount = case when p_overwrite or coalesce(public.marketplace_variant_hpp_mappings.hpp_amount, 0) = 0 then excluded.hpp_amount else public.marketplace_variant_hpp_mappings.hpp_amount end,
    hpp_per_item = case when p_overwrite or coalesce(public.marketplace_variant_hpp_mappings.hpp_per_item, 0) = 0 then excluded.hpp_per_item else public.marketplace_variant_hpp_mappings.hpp_per_item end,
    target_margin_percent = case
      when coalesce(public.marketplace_variant_hpp_mappings.target_margin_percent, 0) = 0 then excluded.target_margin_percent
      else public.marketplace_variant_hpp_mappings.target_margin_percent
    end,
    updated_at = now();
  ...
  return jsonb_build_object(
    ...
    'message', format('%s HPP mapping disinkronkan dari SKU mapping.', v_upserted)
  );
end;
$$;
