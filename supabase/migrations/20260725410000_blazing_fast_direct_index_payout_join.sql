-- Migration: 20260725410000_blazing_fast_direct_index_payout_join.sql
-- Direct indexed join from valid_orders to marketplace_finance_reports on marketplace_order_id and order_id to drop execution time below 300ms.
...
begin
  ...
  finance_payout as (
    ...
    join public.marketplace_finance_reports fr on fr.tenant_id = vo.tenant_id and (fr.marketplace_order_id = vo.marketplace_order_id or fr.order_id = vo.order_key)
    where (p_account_id is null or fr.marketplace_account_id = p_account_id)
    group by vo.tenant_id, vo.order_key
  ),
  matched as (
    select
  ...
  v_res := jsonb_build_object(
    'ok', true,
    'source', 'finance_dashboard_snapshot_core_20260625_fixed_v9',
