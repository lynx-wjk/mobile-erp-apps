...
begin
  ...
  if v_tenant_id is not null then
    select
      coalesce(jsonb_agg(to_jsonb(e) order by coalesce(e.expense_date, e.paid_at, e.created_at::date) desc, e.created_at desc), '[]'::jsonb),
    ...
    select
      coalesce(jsonb_agg(to_jsonb(p) order by coalesce(p.tanggal, p.created_at::date) desc, p.created_at desc), '[]'::jsonb),
  ...
  v_base := jsonb_set(v_base, '{source}', to_jsonb('finance_dashboard_snapshot+tenant_overlay_no_dart_20260625'), true);
