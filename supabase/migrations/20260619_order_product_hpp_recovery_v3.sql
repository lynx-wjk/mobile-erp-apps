-- v3 recovery: fast order list response shape, HPP schema compatibility,
-- safe SKU map seeding from historical live items, and HPP sync helpers.

alter table if exists public.marketplace_variant_hpp_mappings add column if not exists hpp numeric;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists hpp_amount numeric;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists hpp_per_item numeric;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists target_margin_percent numeric;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists source text;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists marketplace_product_id text;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists marketplace_sku_id text;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists marketplace_seller_sku text;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists marketplace_product_name text;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists marketplace_variant_name text;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists local_product_id uuid;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists local_sku text;
alter table if exists public.marketplace_variant_hpp_mappings add column if not exists local_product_name text;

alter table if exists public.marketplace_sku_maps add column if not exists marketplace_product_name text;
alter table if exists public.marketplace_sku_maps add column if not exists marketplace_variant_name text;
alter table if exists public.marketplace_sku_maps add column if not exists marketplace_variation_name text;
alter table if exists public.marketplace_sku_maps add column if not exists local_product_name text;
alter table if exists public.marketplace_sku_maps add column if not exists product_id uuid;
alter table if exists public.marketplace_sku_maps add column if not exists local_product_id uuid;
alter table if exists public.marketplace_sku_maps add column if not exists local_sku text;
alter table if exists public.marketplace_sku_maps add column if not exists marketplace_seller_sku text;
alter table if exists public.marketplace_sku_maps add column if not exists marketplace_sku text;
alter table if exists public.marketplace_sku_maps add column if not exists sync_enabled boolean default false;
alter table if exists public.marketplace_sku_maps add column if not exists status text default 'active';
...
create index if not exists idx_products_tenant_sku_name
  on public.products(tenant_id, kode_sku, nama_barang);
...
begin
  with filtered_orders as (
    ...
    where o.tenant_id = p_tenant_id
      ...
      and (p_status is null or p_status = '' or p_status = 'all' or coalesce(to_jsonb(o)->>'stock_action_status', 'pending') = p_status)
  ...
  item_source as (
    select i.marketplace_order_id, to_jsonb(i) as ij
  ...
  item_counts as (
    select
      ...
      count(*) filter (where coalesce(ij->>'local_sku', '') <> '' or coalesce(ij->>'product_id', '') <> '')::int as mapped_item_count,
      count(*) filter (where coalesce(ij->>'local_sku', '') = '' and coalesce(ij->>'product_id', '') = '')::int as unmapped_item_count,
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'has_cancel_request', case lower(coalesce(fo.oj->>'has_cancel_request', 'false')) when 'true' then true when 't' then true when '1' then true else false end,
...
create or replace function public.marketplace_auto_seed_sku_maps_from_live_items(
  p_tenant_id uuid,
  p_marketplace_account_id uuid default null,
...
begin
  with item_keys as (
    select
      ...
      o.marketplace,
      max(nullif(coalesce(i.marketplace_product_id, ''), '')) as marketplace_product_id,
      max(nullif(coalesce(i.marketplace_sku_id, ''), '')) as marketplace_sku_id,
      max(nullif(coalesce(i.marketplace_seller_sku, i.seller_sku, i.marketplace_sku, ''), '')) as marketplace_seller_sku,
      max(nullif(coalesce(i.marketplace_sku, i.marketplace_seller_sku, i.seller_sku, ''), '')) as marketplace_sku,
      max(nullif(coalesce(i.local_sku, i.marketplace_sku, i.marketplace_seller_sku, i.seller_sku, ''), '')) as candidate_text,
      max(nullif(coalesce(i.marketplace_product_name, i.product_name, i.marketplace_sku, ''), '')) as marketplace_product_name,
      max(nullif(coalesce(i.marketplace_variant_name, i.variant_name, ''), '')) as marketplace_variant_name,
      count(*) as item_rows
    from public.marketplace_order_items i
    join public.marketplace_orders o on o.marketplace_order_id = i.marketplace_order_id
    where i.tenant_id = p_tenant_id
      and (p_marketplace_account_id is null or i.marketplace_account_id = p_marketplace_account_id)
      and coalesce(i.marketplace_product_id, '') <> ''
      and coalesce(i.marketplace_sku_id, '') <> ''
    group by i.tenant_id, i.marketplace_account_id, o.marketplace, i.marketplace_product_id, i.marketplace_sku_id
    order by count(*) desc
  ...
  candidates as (
    select
      ...
      p.kode_sku,
      p.nama_barang,
      count(*) over (partition by k.tenant_id, k.marketplace_account_id, k.marketplace_product_id, k.marketplace_sku_id) as match_count
    from item_keys k
    join public.products p
      on p.tenant_id = k.tenant_id
     and coalesce(p.status, 'active') = 'active'
     and (
       lower(btrim(p.kode_sku)) = lower(btrim(k.candidate_text))
       or lower(btrim(p.nama_barang)) = lower(btrim(k.candidate_text))
       or lower(btrim(p.kode_sku)) = lower(btrim(k.marketplace_seller_sku))
       or lower(btrim(p.nama_barang)) = lower(btrim(k.marketplace_product_name))
     )
  )
  ...
  where c.match_count = 1
    and not exists (
      ...
      where m.tenant_id = c.tenant_id
        and m.marketplace_account_id = c.marketplace_account_id
        and m.marketplace_product_id = c.marketplace_product_id
        and m.marketplace_sku_id = c.marketplace_sku_id
  ...
  return jsonb_build_object(
    'ok', true,
    'inserted', v_inserted,
    'message', format('%s SKU mapping dibuat otomatis dari item live dengan match lokal unik.', v_inserted)
  );
end;
$$;

grant execute on function public.marketplace_auto_seed_sku_maps_from_live_items(uuid, uuid, integer)
  to authenticated, service_role;
