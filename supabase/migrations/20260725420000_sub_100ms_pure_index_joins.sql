-- Migration: 20260725420000_sub_100ms_pure_index_joins.sql
-- Replaces OR join with 2 separate pure B-tree index joins (finance_payout_by_id and finance_payout_by_key) to drop execution time below 300ms.
...
begin
  ...
  finance_payout_by_id as (
    ...
    group by fr.tenant_id, fr.marketplace_order_id
  ),
  finance_payout_by_key as (
    ...
    group by fr.tenant_id, fr.order_id
  ),
  matched as (
    select
  ...
  v_res := jsonb_build_object(
    'ok', true,
    'source', 'finance_dashboard_snapshot_core_20260625_fixed_v10',
