...
const FUNCTION_VERSION = 'marketplace-finance-pull-overwrite-auto-payout-5m-2026-06-06';
const corsHeaders = {
...
async function callTikTokFinanceService(args) {
  const params = {
    mode: text(args.body.mode) || 'today_yesterday',
    ...
    max_orders: numberIn(args.body.max_orders, 1, 20, 20),
    max_batches_per_job: numberIn(args.body.max_batches_per_job, 1, 3, 3),
    max_statements: numberIn(args.body.max_statements, 1, 8, 4),
    max_transactions: numberIn(args.body.max_transactions, 1, 80, 60),
    max_order_details: numberIn(args.body.max_order_details, 1, 40, 20),
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    for (const tenantId of tenants){
      const result = await callTikTokFinanceService({
        supabaseUrl,
        serviceRoleKey,
        cronSecret,
        tenantId,
      ...
      const message = text(result.message) || 'Auto payout selesai.';
      await touchFinanceAutoSettings(admin, tenantId, message);
