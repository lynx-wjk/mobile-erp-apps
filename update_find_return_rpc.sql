...
begin
  ...
  with matches as (
    ...
    left join public.marketplace_accounts ma on ma.marketplace_account_id = r.marketplace_account_id
    ...
    left join public.marketplace_accounts ma on ma.marketplace_account_id = c.marketplace_account_id
    ...
    select 3 as priority,
      ...
      jsonb_build_object(
        ...
        'recommended_action', case when o.return_case_id is not null or o.return_review_status is not null then 'REVIEW_REFUND_RETURN' else 'REVIEW_CANCEL_WITH_REAL_RESI' end,
    ...
    left join public.marketplace_accounts ma on ma.marketplace_account_id = o.marketplace_account_id
  ...
  into v_rows
  from (select * from matches order by priority, sort_time desc limit 30) x;

  return jsonb_build_object('ok', true, 'query', p_resi, 'rows', v_rows, 'total', jsonb_array_length(v_rows));
end;
$$;
