-- Comprehensive RPC patch fix_date_filters_v5.sql
-- 1. Sets true API Omzet per marketplace from marketplace_orders
-- 2. Calculates HPP per marketplace for settled orders
-- 3. Calculates Estimated Unpaid HPP for orders waiting for payout
-- 4. Excludes duplicate statement summary rows
-- 5. Preserves strict tenant isolation for approved purchases
...
begin
  ...
  -- 3. Calculate HPP for settled finance orders
  with hpp_sku as (
    ...
    where coalesce(is_active, true) = true and (v_tenant_id is null or tenant_id = v_tenant_id) and nullif(marketplace_sku_id, '') is not null
    group by 1, 2, 3
  ),
  hpp_seller as (
    select tenant_id, marketplace_account_id, lower(nullif(marketplace_seller_sku, '')) as marketplace_seller_sku,
           max(coalesce(hpp_amount, hpp, hpp_per_item, 0))::numeric as hpp
    from public.marketplace_variant_hpp_mappings
    where coalesce(is_active, true) = true and (v_tenant_id is null or tenant_id = v_tenant_id) and nullif(marketplace_seller_sku, '') is not null
    group by 1, 2, 3
  ),
  settled_items as (
    select fr.finance_report_id,
           coalesce(oi.quantity, oi.qty, 1)::numeric as qty,
           coalesce(hs.hpp, hsel.hpp, 0)::numeric as unit_hpp
    from public.marketplace_finance_reports fr
    join public.marketplace_orders o on o.tenant_id = fr.tenant_id and (o.order_id = fr.order_id or o.marketplace_order_id = fr.marketplace_order_id)
    join public.marketplace_order_items oi on oi.marketplace_order_id = o.marketplace_order_id
    left join hpp_sku hs on hs.marketplace_account_id = fr.marketplace_account_id and hs.marketplace_sku_id = lower(nullif(oi.marketplace_sku_id, ''))
    left join hpp_seller hsel on hsel.marketplace_account_id = fr.marketplace_account_id and hsel.marketplace_seller_sku = lower(nullif(oi.marketplace_seller_sku, ''))
  ...
  select coalesce(sum(qty * unit_hpp), 0)::numeric into v_hpp_total from settled_items;
  ...
  with unpaid_orders as (
    ...
    from public.marketplace_orders o
    left join public.marketplace_finance_reports fr on fr.tenant_id = o.tenant_id and (fr.order_id = o.order_id or fr.marketplace_order_id = o.marketplace_order_id)
    where (v_tenant_id is null or o.tenant_id = v_tenant_id)
      ...
      and not (lower(concat_ws(' ', o.order_status, o.status, o.raw_order->>'status')) ~ '(cancel|canceled|cancelled|batal|dibatalkan|returned|return|refund|rts|gagal|failed|closed)')
      and fr.finance_report_id is null
  ),
  hpp_sku as (
    select tenant_id, marketplace_account_id, lower(nullif(marketplace_sku_id, '')) as marketplace_sku_id,
           max(coalesce(hpp_amount, hpp, hpp_per_item, 0))::numeric as hpp
  ...
  unpaid_items as (
    select uo.marketplace_order_id,
           coalesce(oi.quantity, oi.qty, 1)::numeric as qty,
           coalesce(hs.hpp, hsel.hpp, 0)::numeric as unit_hpp
    from unpaid_orders uo
    join public.marketplace_order_items oi on oi.marketplace_order_id = uo.marketplace_order_id
  ...
  select coalesce(sum(qty * unit_hpp), 0)::numeric into v_estimated_unpaid_hpp from unpaid_items;
  ...
  -- 6. Build Marketplace Cards with True API Omzet & Calculated Settled HPP
  ...
  fin_hpp as (
    select
      ...
      fr.marketplace_account_id,
      sum(coalesce(oi.quantity, oi.qty, 1) * coalesce(hs.hpp, hsel.hpp, 0))::numeric as hpp_total
    from public.marketplace_finance_reports fr
    join public.marketplace_orders o on o.tenant_id = fr.tenant_id and (o.order_id = fr.order_id or o.marketplace_order_id = fr.marketplace_order_id)
    join public.marketplace_order_items oi on oi.marketplace_order_id = o.marketplace_order_id
    left join hpp_sku hs on hs.marketplace_account_id = fr.marketplace_account_id and hs.marketplace_sku_id = lower(nullif(oi.marketplace_sku_id, ''))
  ...
  rows as (
    select
      ...
      coalesce(fh.hpp_total, f.hpp_total, 0) as hpp_total,
    ...
    left join fin_hpp fh on fh.marketplace = k.marketplace and fh.marketplace_account_id = k.marketplace_account_id
  )
  select
  ...
  return jsonb_build_object(
    'ok', true,
    'version', 'finance_dashboard_snapshot_api_omzet_unpaid_hpp_v5_20260727',
    'wrapper_version', 'finance_dashboard_snapshot_api_omzet_unpaid_hpp_v5_20260727',
...
begin
  ...
  if jsonb_typeof(v_by_mp) = 'array' and jsonb_array_length(v_by_mp) > 0 then
    for v_elem in select * from jsonb_array_elements(v_by_mp) loop
      ...
      v_hpp := coalesce((v_elem->>'hpp_total')::numeric, (v_elem->>'total_hpp')::numeric, 0);
      ...
      v_elem := jsonb_set(v_elem, '{net_profit}', to_jsonb(v_payout - v_hpp));
      v_elem := jsonb_set(v_elem, '{profit}', to_jsonb(v_payout - v_hpp));
      v_elem := jsonb_set(v_elem, '{margin_percent}', to_jsonb(case when v_payout <> 0 then round(((v_payout - v_hpp) / v_payout) * 100, 2) else 0 end));
